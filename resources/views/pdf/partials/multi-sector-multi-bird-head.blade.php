@php
        ...
        $formatEspecieIdentificada = static function (array $especie): string {
            $nombreComun = trim((string) ($especie['nombre_comun'] ?? ''));
            $descripcionAve = trim((string) ($especie['descripcion'] ?? $especie['nombre_cientifico'] ?? ''));

            if ($nombreComun === '') {
                return '';
            }

            if ($descripcionAve !== '' && $descripcionAve !== '—') {
                return "{$nombreComun} ({$descripcionAve})";
            }

            return $nombreComun;
        };

        $partesEspeciesIdentificadas = collect($especies_identificadas_tabla ?? [
            [
            ...
            ->map(fn (array $especie): string => $formatEspecieIdentificada($especie))
            ->filter(fn (string $parte): bool => $parte !== '')
            ->values();

        $especies_identificadas_linea = match (true) {
            $partesEspeciesIdentificadas->isEmpty() => '—',
            $partesEspeciesIdentificadas->count() === 1 => $partesEspeciesIdentificadas->first().'.',
            default => $partesEspeciesIdentificadas->slice(0, -1)->implode(', ').' y '.$partesEspeciesIdentificadas->last().'.',
        };

        $tablaPoblacionInicialPorAve = collect($tabla_poblacion_inicial_por_ave ?? [
            [
                'nombre_comun' => 'Paloma doméstica',
                'descripcion' => 'Columba livia',
                'poblacion_inicial' => 0,
        ...
        $cantidad_primera_visita_historica = (int) (
            $tablaPoblacionInicialPorAve->first()['poblacion_inicial']
            ?? $poblacion_inicial
            ?? 0
        );
        ...
        $visitColumnsRegistroDynamicasPorTabla = 4;

        $visitColumnsBasicas = $visitColumns->filter(
            fn (array $column): bool => in_array($column['key'], ['visit_date_init', 'visit_date_end', 'employee.name'], true),
        )->values();

        $visitColumnsDynamicas = $visitColumns->filter(
            fn (array $column): bool => str_starts_with($column['key'], 'qty_'),
        )->values();

        $visitColumnObservacion = $visitColumns->first(
            fn (array $column): bool => $column['key'] === 'visit_observation',
        );

        if ($visitColumnsDynamicas->count() <= $visitColumnsRegistroDynamicasPorTabla) {
            $visitTableChunks = collect([
                $visitColumnsBasicas
                    ->concat($visitColumnsDynamicas)
                    ->when(
                        $visitColumnObservacion !== null,
                        fn (\Illuminate\Support\Collection $columns) => $columns->push($visitColumnObservacion),
                    )
                    ->values(),
            ]);
        } else {
            $visitTableChunks = $visitColumnsDynamicas
                ->chunk($visitColumnsRegistroDynamicasPorTabla)
                ->values()
                ->map(function (\Illuminate\Support\Collection $dynamicChunk) use (
                    $visitColumnsBasicas,
                    $visitColumnObservacion,
                ): \Illuminate\Support\Collection {
                    $columns = $visitColumnsBasicas->concat($dynamicChunk)->values();

                    if ($visitColumnObservacion !== null) {
                        $columns = $columns->push($visitColumnObservacion);
                    }
        ...
        $visitTableChunksHasContinued = $visitTableChunks->count() > 1;
        ...
        $detalleServicioPorUbicacion = $detalle_servicio_por_ubicacion ?? $report_service_details_by_location ?? [];
        $detalleServicioPorUbicacionSecciones = $detalleServicioPorUbicacion['sections'] ?? [];
...
html, body { margin: 0; background: #ffffff; }
    ...
    .report-objective-methodology-page--has-table-continued {
        page-break-after: auto;
        break-after: auto;
    }

    .report-objective-methodology-page--visit-table-continued {
