        ...
        <table class="report-initial-situation-page__table">
            ...
            <tbody>
                @foreach ($tablaPoblacionInicialPorAve as $filaPoblacionInicial)
                    <tr>
                        <td>
                            {{ $filaPoblacionInicial['nombre_comun'] ?? '' }}@if (($filaPoblacionInicial['descripcion'] ?? '') !== '' && ($filaPoblacionInicial['descripcion'] ?? '') !== '—') ({{ $filaPoblacionInicial['descripcion'] }})@endif
                        </td>
...
<section @class([
    ...
    'report-objective-methodology-page--has-table-continued' => $visitTableChunksHasContinued ?? false,
])>
    ...
    <div class="report-objective-methodology-page__content">
        ...
        @if ($visitRows->isNotEmpty() && $visitColumns->isNotEmpty())
            @php($visitColumnsChunk = $visitTableChunks->first())
            @include('pdf.partials.report-objective-methodology-visit-table-chunk')
        @else
...
@foreach ($visitTableChunks->skip(1) as $visitColumnsChunk)
    <section class="report-objective-methodology-page report-objective-methodology-page--visit-table-continued">
        @include('pdf.partials.report-objective-methodology-visit-table-chunk')
    </section>
@endforeach
...
<section class="report-current-situation-and-conclusions-page" aria-label="Situacion actual y conclusiones">
    ...
    <div class="report-current-situation-and-conclusions-page__content">
        ...
        @if ($reductionPercentage !== null)
            <p>La población de aves ha disminuido en un {{ $reductionPercentage }}%.</p>
        @endif
